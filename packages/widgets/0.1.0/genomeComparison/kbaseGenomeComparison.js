...
define([
    ...
    'kb_service/utils',
    'd3'
], function (Promise, $, html, Workspace, apiUtils, d3) {
    'use strict';
                ...
                return this.validateParams(input)
                    .then(function () {
                        return config.runtime.requests([
                            ...
                            ['config', {property: 'services.genome_comparison.url'}],
                            ['authToken']
                        ]);
                    })
...
//            },
            ...
            loadCmp: function ($container, kbws) {
                var self = this;
                $container.empty();
                $container.append(html.loading('loading comparison data...'));
                return kbws.get_object_subset([
                    {
                        ref: self.cmp.genome1ref,
                        included: ["scientific_name"]
                    },
                    {
                        ref: self.cmp.genome2ref,
                        included: ["scientific_name"]
                    }])
                    .then(function (data) {
                        ...
                        table.append(createTableRow("Comparison object", self.cmp_ref));
                    ...
                    .catch(function (data) {
                        ...
                        tdElem.html("Error accessing genome objects: " + self.errorMessage(data));
                    });
            },
            ...
            render: function ($container, params, workspaceUrl, authToken) {
                ...
                var kbws = new Workspace(workspaceUrl, {
                    token: authToken
                });
                ...
                function dataIsReady() {
                    kbws.get_objects([{ref: params.objectRef}])
                        .then(function (data) {
                            ...
                            self.objectInfo = apiUtils.objectInfoToObject(info);
                            return self.loadCmp($container, kbws);
                        })
                        .catch(function (err) {
                            var tdElem = $('#' + self.pref + 'job');
...
//
//        runtime.on('start', function (data) {
//            // Store local reference to the node given to us.
//            // container = data.node;
